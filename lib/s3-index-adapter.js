...
var Adapter = CoreObject.extend({
      ...
      .then(function(versions) {
        if (versions.indexOf(key) === -1) {
          ...
          return self._getObjectBody(key).then(function(body) {
            return self._putObjectData('index', body).then(function() {
              return key;
            });
          });
        }
  ...
  _getObjectBody: function(key) {
    ...
    return new CLIPromise(function(resolve, reject) {
      self.client.getObject({ Bucket: self.connection.bucket, Key: key + '.html' }, function(err, data) {
        if (err) {
          return reject(new SilentError(err));
        }
  ...
  _putObjectData: function(key, value) {
    ...
    return new CLIPromise(function(resolve, reject) {
      self.client.putObject({ Bucket: self.connection.bucket, Key: key + '.html', Body: value }, function(err, data) {
        if (err) {
