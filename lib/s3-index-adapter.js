...
var Adapter = CoreObject.extend({
  ...
  _listVersions: function(count) {
    ...
    return new CLIPromise(function(resolve, reject) {
      this.client.listObjects({ Bucket: self.connection.bucket }, function(err, data) {
        if (err) {
  ...
  _getObjectBody: function(key) {
    ...
    return new CLIPromise(function(resolve, reject) {
      self.client.getObject({ Bucket: self.connection.bucket, Key: key }, function(err, data) {
        if (err) {
          return reject(new SilentErrorError(err));
        }
  ...
  _upload: function(key, value) {
    ...
    return new CLIPromise(function(resolve, reject) {
      self.client.putObject({ Bucket: self.connection.bucket, Key: key, Body: value }, function(err, data) {
        if (err) {
          return reject(new SilentErrorError(err));
        }
